...
SoftwareSerial mySerial1(35, 34); // RX, TX (cek kembali apakah pin benar)
...
void loop() {
  ...
  if (currentMillis - previousMillis >= sendInterval) {
    // Listen Before Talk: Check for activity before sending
    int packetSize = LoRa.parsePacket();
    if (packetSize == 0) { // No packet received, it's clear to send
